...
type ContractsForVersion<V extends JBVersion> = V extends 4
  ...
  : V extends 5
    ...
    : V extends 6
      ? keyof (typeof jbContractAddress)["6"]
      : never;
...
export async function getPrimaryNativeTerminal(
  ...
  const address = getJBContractAddress(
  ...
  const primaryNativeTerminal = await directory.read.primaryTerminalOf([
    projectId,
    NATIVE_TOKEN,
  ]);
  if (primaryNativeTerminal !== zeroAddress) return primaryNativeTerminal;

  return await directory.read.primaryTerminalOf([
    projectId,
